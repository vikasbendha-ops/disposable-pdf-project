...
const Login = () => {
  ...
  const { login, user, resendVerificationEmail, verifyTwoFactorChallenge } = useAuth();
  ...
  const [twoFactorChallenge, setTwoFactorChallenge] = useState(location.state?.twoFactorPending || null);
  ...
  React.useEffect(() => {
    ...
    if (location.state?.twoFactorPending) {
      setTwoFactorChallenge(location.state.twoFactorPending);
      setTwoFactorCode('');
      setError('');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result?.requires_2fa) {
        ...
        setTwoFactorCode('');
        toast.success(t('authFlow.twoFactorPrompt'));
        return;
      }
      ...
      setError(detail);
      const needsVerification = err.response?.status === 403 || /verify your email/i.test(detail);
  ...
  const handleTwoFactorSubmit = async (e) => {
    ...
    try {
      await verifyTwoFactorChallenge(twoFactorChallenge?.challenge_token, twoFactorCode);
      toast.success(t('authFlow.welcomeBackToast'));
      navigate(from, { replace: true });
    } catch (err) {
      setError(err.response?.data?.detail || t('authFlow.twoFactorInvalid'));
      toast.error(t('authFlow.twoFactorFailed'));
    } finally {
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        ...
        >
          ...
          <h2 className="font-heading text-3xl font-bold text-stone-900 mb-2">
            {twoFactorChallenge ? t('authFlow.twoFactorTitle') : t('auth.signInTitle')}
          </h2>
          <p className="text-stone-600 mb-8">
            {twoFactorChallenge ? t('authFlow.twoFactorDescription') : t('auth.signInDesc')}
          </p>

          {registrationPendingVerification && !twoFactorChallenge && (
          ...
          {showVerificationHelp && !twoFactorChallenge && (
            ...
            <form onSubmit={handleTwoFactorSubmit} className="space-y-6">
              <div className="rounded-lg border border-emerald-200 bg-emerald-50/70 p-4">
                <p className="font-medium text-stone-900">
                  {t('authFlow.twoFactorForAccount')}: {twoFactorChallenge?.email || email}
                </p>
                <p className="text-sm text-stone-600 mt-1">
                  {t('authFlow.twoFactorDescription')}
                </p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="two-factor-code" className="text-sm font-semibold text-stone-700">
                  {t('authFlow.twoFactorCodeLabel')}
                </Label>
                <Input
                  id="two-factor-code"
                  type="text"
                  ...
                  placeholder="123456"
                  className="h-12 tracking-[0.35em] text-center text-lg bg-white border-stone-200 focus:border-emerald-600 focus:ring-emerald-600"
                  required
                  maxLength={6}
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                <Button
                  type="submit"
                  className="flex-1 h-12 bg-emerald-900 hover:bg-emerald-800 text-white"
                  disabled={twoFactorLoading || twoFactorCode.length !== 6}
                  data-testid="two-factor-submit-btn"
                >
                  {twoFactorLoading ? t('authFlow.twoFactorVerifying') : t('authFlow.twoFactorSubmit')}
                </Button>
                <Button type="button" variant="outline" className="h-12" onClick={handleCancelTwoFactor}>
                  {t('authFlow.twoFactorBack')}
                </Button>
              </div>
            </form>
          ) : (
