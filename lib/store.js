...
class DocumentCollection {
  ...
  async _fetchRows(query = null, options = {}) {
    ...
    if (hasQuery && canUseJsonContainmentQuery(normalizedQuery)) {
      const params = [this.name, isoJson(normalizedQuery)];
      let sql =
        "SELECT id, doc FROM app_documents WHERE collection = $1 AND doc @> $2::jsonb ORDER BY id ASC";
      if (limit) {
        params.push(limit);
        sql += " LIMIT $3";
      }
      result = await pool.query(sql, params);
    } else if (!hasQuery && limit) {
      result = await pool.query(
        "SELECT id, doc FROM app_documents WHERE collection = $1 ORDER BY id ASC LIMIT $2",
        [this.name, limit],
      );
    } else {
      result = await pool.query(
